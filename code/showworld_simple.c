...
void showworld_update(SHOWWORLD *sw, void *w) { 
    Texture2D Hum = LoadTexture("Human.png"); // Texture loading
    Texture2D Zom = LoadTexture("Zombie.png"); // Texture loading
            ...
            switch (ag_type) {
                ...
                case Zombie:
                    if (playable) {
                        ...
                        DrawTexture(Zom, x * (GetScreenWidth() / sw->xdim) + 10, y * (GetScreenHeight() / sw->ydim) + 5, WHITE);
                    } else {
